...
public class FileServiceImpl extends BaseServiceImpl<FileMapper, File> implements FileService {
    ...
    public String uploadImage(MultipartFile file, OssModuleEnum module, Long userId) {
        try {
            // 1. 检验用户是否有正常操作的权限
            if (userUtil.getUserById(userId) == null) {
            ...
            throw new BusinessException(OssErrorCode.OSS_MULTIPART_FILE_UPLOAD_FAILS);
        }
    }
    ...
    public void downloadToResponse(String objectName, HttpServletResponse response) {
        try {
            ...
            throw new BusinessException(OssErrorCode.OSS_FILE_DOWNLOAD_FAILS);
        }
    ...
    public String generatePresignedUrl(String objectName) {
        try {
            ...
            throw new BusinessException(OssErrorCode.OSS_PRESIGNED_URL_GENERATE_FAILS);
        }
    }
    ...
    public void deleteFile(String objectName, Long userId) {
        try {
            ...
            throw new BusinessException(OssErrorCode.OSS_FILE_DELETE_FAILS);
        }
    }
    ...
    public List<String> listAllFiles(String bucketName, Long userId) {
        try {
            // 1. 检验用户是否有正常操作的权限
            if (!userUtil.verifyRole(userId, UserRoleEnum.ADMIN)) {
            ...
            throw new BusinessException(OssErrorCode.OSS_SERVICE_FAILS);
        }
    }

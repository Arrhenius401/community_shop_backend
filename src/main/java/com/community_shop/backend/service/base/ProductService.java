...
public interface ProductService {
     ...
     * @return ResultDTO<PageResult<Product>> 成功返回分页商品列表，失败返回错误信息
     * @see com.community_shop.backend.mapper.ProductMapper#selectByKeyword(String, int, int)
     */
    ResultDTO<PageResult<Product>> selectProductByKeyword(String keyword, PageParam pageParam);
     ...
     * @return ResultDTO<Boolean> 成功返回true，失败返回错误信息（如"无权限更新"）
     * @see com.community_shop.backend.mapper.ProductMapper#updateById(Product)
     */
    ResultDTO<Boolean> updateProductInfo(Long productId, ProductUpdateVO productUpdateVO, Long sellerId);
     ...
     */
    ResultDTO<Boolean> deleteProductById(Long productId, Long sellerId);
     ...
     * @param sellerId 卖家ID
     * @return ResultDTO<String> 成功返回"发布成功"，失败返回错误信息（如"成色非法"）
     * @see #insertProduct(Product)
     * @see UserService#selectUserById(Long)
     * @see com.community_shop.backend.component.enums.ProductConditionEnum （商品成色枚举）
     */
    ResultDTO<String> publishProduct(ProductVO productVO, Long sellerId);
     ...
     * @return ResultDTO<Integer> 成功返回更新后的库存，失败返回错误信息（如"库存不足"）
     * @see com.community_shop.backend.mapper.ProductMapper#updateStock(Long, int)
     * @see #selectProductById(Long)
     */
    ResultDTO<Integer> updateStock(Long productId, Integer stockChange, String reason);
     ...
     * @param pageParam 分页参数（页码、每页条数）
     * @return ResultDTO<PageResult<Product>> 成功返回含卖家好评率的分页商品列表，失败返回错误信息
     ...
     */
    ResultDTO<PageResult<Product>> selectProductByCondition(String category, Double minPrice, Double maxPrice, PageParam pageParam);
}

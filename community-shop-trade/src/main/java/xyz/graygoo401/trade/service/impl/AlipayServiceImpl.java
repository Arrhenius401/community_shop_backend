...
import xyz.graygoo401.api.trade.enums.PayStatusEnum;
...
import xyz.graygoo401.trade.config.AlipayConfig;
...
import xyz.graygoo401.trade.dao.mapper.PaymentMapper;
import xyz.graygoo401.trade.exception.error.OrderErrorCode;
import xyz.graygoo401.trade.exception.error.PaymentErrorCode;
import xyz.graygoo401.trade.service.base.PaymentService;
...
public class AlipayServiceImpl extends BaseServiceImpl<PaymentMapper, Payment> implements PaymentService {
    ...
    public String generatePaymentUrl(Long orderId, Long userId) {
        ...
        if (!OrderStatusEnum.PENDING_PAYMENT.equals(order.getStatus())) {
            throw new BusinessException(OrderErrorCode.ORDER_STATUS_INVALID, "订单状态异常，仅待支付订单可发起支付");
        }
        // 校验用户ID
        if (!order.getBuyerId().equals(userId)) {
            throw new BusinessException(PaymentErrorCode.PAYMENT_USER_NOT_MATCH);
        }
        ...
        try {
            ...
            if (response.isSuccess()) {
                ...
                throw new BusinessException(PaymentErrorCode.PAYMENT_GENERATE_URL_FAILS, "生成支付链接失败：" + response.getMsg());
            }
        } catch (BusinessException e) {
          throw e;
        } catch (AlipayApiException e) {
            log.error("支付宝API调用异常：" + e.getErrMsg());
            throw new BusinessException(PaymentErrorCode.PAYMENT_ALIPAY_FAILS, "支付宝API调用异常");
        } catch (Exception e) {
            log.error("支付异常：" + e.getMessage());
            throw new BusinessException(PaymentErrorCode.PAYMENT_ALIPAY_FAILS, "支付异常");
        }
    }
    ...
    public Boolean handlePayCallback(Map<String, String> params) {
        try {
            ...
            if (!signVerified) {
                throw new BusinessException(PaymentErrorCode.PAYMENT_CALLBACK_VALIDATE_FAILS);
            }
            ...
            if (!"TRADE_SUCCESS".equals(tradeStatus)) {
                return true; // 非成功状态无需处理，返回 true 避免重试
            }
            ...
            if (payment == null) {
                throw new BusinessException(PaymentErrorCode.PAYMENT_NOT_EXISTS, "支付记录不存在，订单号：" + outTradeNo);
            }
            ...
            throw new BusinessException(PaymentErrorCode.PAYMENT_CALLBACK_FAILS);
        }
    }
}

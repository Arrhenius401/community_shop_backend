...
import com.community_shop.backend.service.impl.EvaluationServiceImpl;
...
public class EvaluationServiceTest {
    ...
    // 测试数据
    private User testBuyer;        // 买家用户（评价发起者）
    private User testAdmin;        // 管理员用户（用于删除评价权限场景）
    private User testSeller;       // 卖家用户（被评价方，无评价操作权限）
    ...
    void testSubmitEvaluation_Success() {
        // 1. 模拟依赖行为
        // 校验买家存在
        when(userService.getById(1L)).thenReturn(testBuyer);
        // 校验订单存在且已完成
...
//            dto.setProduct(productDTO);
        ...
        // 4. 验证依赖调用
        verify(userService, times(1)).getById(1L); // 校验买家存在
    ...
    void testSubmitEvaluation_Fail_OrderNotCompleted() {
        ...
        // 5. 验证异常信息
        assertEquals(ErrorCode.ORDER_NOT_COMPLETED.getCode(), exception.getCode());
    ...
    void testSubmitEvaluation_Fail_OrderAlreadyEvaluated() {
        ...
        // 3. 验证异常信息
        assertEquals(ErrorCode.ORDER_ALREADY_EVALUATED.getCode(), exception.getCode());
    ...
    void testUpdateEvaluationContent_Fail_NoPermission() {
        ...
        when(evaluationMapper.selectById(3001L)).thenReturn(testEvaluation);
        when(userService.getById(2L)).thenReturn(testSeller); // 卖家操作
    ...
    void testDeleteEvaluationById_Success_Admin() {
        // 1. 模拟依赖行为
        when(evaluationMapper.selectById(3001L)).thenReturn(testEvaluation); // 评价存在
        when(userService.getById(999L)).thenReturn(testAdmin); // 管理员用户
        ...
        verify(userService, times(1)).getById(999L); // 校验管理员身份
    ...
    void testSearchEvaluationsByQuery_Success() {
        ...
        when(userService.getById(2L)).thenReturn(testSeller);    // 返回卖家用户
...
//        verify(redisTemplate, times(1)).opsForValue().set( // 缓存评分结果
//                eq("evaluation:seller:score:2"),
//                any(SellerScoreDTO.class),
//                eq(3600L), // 缓存1小时
//                eq(TimeUnit.SECONDS)
//        );
    }
...
//        verify(redisTemplate, times(0)).opsForValue().set( // 缓存评分结果
//                eq("evaluation:product:score:2001"),
//                any(ProductScoreDTO.class),
//                eq(3600L), // 缓存1小时
//                eq(TimeUnit.SECONDS)
//        );
    }

...
import xyz.graygoo401.api.infra.dto.message.*;
...
import xyz.graygoo401.infra.convert.MessageConvert;
import xyz.graygoo401.infra.dao.entity.Message;
import xyz.graygoo401.infra.dao.mapper.MessageMapper;
import xyz.graygoo401.infra.exception.error.MessageErrorCode;
import xyz.graygoo401.infra.service.impl.MessageServiceImpl;
...
public class MessageServiceTest {
    ...
    // 测试数据
    private UserDTO testSender;       // 发送者（普通用户）
    private UserDTO testReceiver;     // 接收者（普通用户）
    private UserDTO testSystemUser;   // 系统用户（模拟发送者ID=0）
    ...
    void testSendMessage_Success() {
        // 1. 模拟依赖行为
        when(userUtil.getUserById(1L)).thenReturn(testSender);          // 校验发送者存在
        when(userUtil.getUserById(2L)).thenReturn(testReceiver);        // 校验接收者存在
        ...
        // 4. 验证依赖调用
        verify(userUtil, times(1)).getUserById(1L);          // 校验发送者
        verify(userUtil, times(1)).getUserById(2L);          // 校验接收者
    ...
    void testSendMessage_Fail_ReceiverNotExists() {
        ...
        when(userUtil.getUserById(1L)).thenReturn(testSender);
        when(userUtil.getUserById(2L)).thenReturn(null); // 接收者不存在
        ...
        // 3. 验证异常信息
        assertEquals(MessageErrorCode.RECEIVER_NOT_EXISTS, exception.getErrorCode());
    ...
    void testGetMessageDetail_Success() {
        // 1. 模拟依赖行为
        when(userUtil.getUserById(2L)).thenReturn(testReceiver); // 接收者存在
        ...
        when(userUtil.getUserById(1L)).thenReturn(testSender); // 接收者存在
    ...
    void testUpdateMessageStatus_Fail_NoPermission() {
        ...
        when(userUtil.getUserById(3L)).thenReturn(new UserDTO()); // 操作人ID=3（非接收者）
    ...
    void testGetRecentUnreadPreviews_Success_FromDb() {
        ...
        when(userUtil.getUserById(1L)).thenReturn(testSender); // 普通发送者信息

package xyz.graygoo401.user.service.impl;
...
import xyz.graygoo401.api.user.dto.third.ThirdPartyLoginDTO;
import xyz.graygoo401.api.user.dto.user.*;
import xyz.graygoo401.api.user.enums.LoginTypeEnum;
...
import xyz.graygoo401.user.dao.entity.UserThirdParty;
import xyz.graygoo401.user.dao.mapper.UserMapper;
import xyz.graygoo401.user.dao.mapper.UserThirdPartyMapper;
import xyz.graygoo401.user.exception.error.UserErrorCode;
...
public class UserServiceImpl extends BaseServiceImpl<UserMapper, User> implements UserService {
    ...
    public Boolean register(RegisterDTO registerDTO) {
        try {
            ...
            if (!StringUtils.hasText(registerDTO.getVerifyCode())
                ...
                throw new BusinessException(UserErrorCode.VERIFY_FAILURE);
            }
    ...
    public Boolean registerByEmail(RegisterEmailDTO registerDTO) {
        try {
            ...
            Boolean verifyRes = infraClient.checkEmailVerifyCode(new VerifyEmailDTO(registerDTO.getEmail(), registerDTO.getVerifyCode())).getData();
            if (!Boolean.TRUE.equals(verifyRes)) {
                throw new BusinessException(UserErrorCode.VERIFY_FAILURE);
            }
    ...
    public Boolean registerByPhone (RegisterPhoneDTO registerDTO) {
        try {
            ...
            // 3. 校验验证码（匹配文档“注册校验”要求）
            Boolean verifyRes = infraClient.checkPhoneVerifyCode(new VerifyPhoneDTO(registerDTO.getPhoneNumber(), registerDTO.getVerifyCode())).getData();
            if (!Boolean.TRUE.equals(verifyRes)) {
                throw new BusinessException(UserErrorCode.VERIFY_FAILURE);
            }
    ...
    public Boolean bindEmail (VerifyEmailDTO verifyDTO) {
        try {
            ...
            // 2. 校验验证码
            Boolean verifyRes = infraClient.checkEmailVerifyCode(new VerifyEmailDTO(verifyDTO.getUserId(), verifyDTO.getEmail(), verifyDTO.getVerifyCode())).getData();
            if (!Boolean.TRUE.equals(verifyRes)) {
                throw new BusinessException(UserErrorCode.VERIFY_FAILURE);
            }
    ...
    public Boolean bindPhone (VerifyPhoneDTO verifyDTO) {
        try {
            ...
            // 2. 校验验证码
            Boolean verifyRes = infraClient.checkPhoneVerifyCode(new VerifyPhoneDTO(verifyDTO.getUserId(), verifyDTO.getPhoneNumber(), verifyDTO.getVerifyCode())).getData();
            if (!Boolean.TRUE.equals(verifyRes)) {
                throw new BusinessException(UserErrorCode.VERIFY_FAILURE);
            }
    ...
    public Boolean updatePasswordByEmail (PasswordUpdateEmailDTO verifyDTO) {
        try {
            ...
            if (!userMapper.selectByEmail(verifyDTO.getEmail()).getUserId().equals(verifyDTO.getUserId())) {
                throw new BusinessException(UserErrorCode.EMAIL_NOT_BELONG_TO_USER);
            }
    ...
    public Boolean updatePasswordByPhone (PasswordUpdatePhoneDTO verifyDTO) {
        try {
            ...
            if (!userMapper.selectByPhone(verifyDTO.getPhoneNumber()).getUserId().equals(verifyDTO.getUserId())) {
                throw new BusinessException(UserErrorCode.PHONE_NOT_BELONG_TO_USER);
            }
    ...
    public LoginResultDTO login(LoginDTO loginDTO) {
        try {
            ...
            String token = tokenUtil.generateToken(user.getUserId(), user.getRole().getCode(), user.getStatus().getCode());
            ...
            log.error("用户登录失败", e);
            throw new BusinessException(SystemErrorCode.DATA_QUERY_FAILED);
        }

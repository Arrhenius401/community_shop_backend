...
public class OrderServiceTest {
    ...
    private UserDTO testBuyerUser;
    private UserDTO testSellerUser;
    private UserDTO testLowCreditUser;
    ...
    private void mockConvertBehavior() {
        ...
        // 用户转换与数据库操作
        when(orderConvert.userDTOToBuyerSimpleDTO(testBuyerUser)).thenAnswer(invocation -> {
        ...
        when(orderConvert.userDTOToSellerSimpleDTO(testSellerUser)).thenAnswer(invocation -> {
    ...
    void testCreateOrder_Success() {
        // 1. 模拟依赖行为
        when(userUtil.getUserById(2L)).thenReturn(testBuyerUser);
        when(userUtil.getUserById(3L)).thenReturn(testSellerUser);
        ...
        // 4. 验证依赖调用
        verify(userUtil, times(2)).getUserById(2L);
...
//        verify(messageService, times(1)).sendMessage(eq(0L), any(MessageSendDTO.class));
    ...
    void testCreateOrder_CreditTooLow() {
        // 1. 模拟依赖行为（低信用分用户）
        when(userUtil.getUserById(4L)).thenReturn(testLowCreditUser);
        when(productService.getById(1001L)).thenReturn(testProduct);
    ...
    void testCreateOrder_StockInsufficient() {
        ...
        // 4. 验证结果
        assertEquals(ProductErrorCode.PRODUCT_STOCK_INSUFFICIENT, exception.getErrorCode());
        verify(orderMapper, never()).insert(any(Order.class));
    }
    ...
    void testCancelOrder_Success_Buyer() {
        ...
        verify(userUtil, times(1)).getUserById(2L);
...
//        verify(messageService, times(1)).sendMessage(eq(0L), any(MessageSendDTO.class));
    ...
    void testCancelOrder_InvalidStatus() {
        ...
        // 4. 验证结果
        assertEquals(OrderErrorCode.ORDER_STATUS_INVALID, exception.getErrorCode());
        verify(orderMapper, never()).updateById(any(Order.class));
    }

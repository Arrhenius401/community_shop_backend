...
import xyz.graygoo401.api.community.enums.PostSortFieldEnum;
...
import xyz.graygoo401.community.service.impl.PostServiceImpl;
...
public class PostServiceTest {
    ...
    private UserDTO testNewUser;
    private Map<Long, UserDTO> testUserMap;
    ...
    private void initTestUsers() {
        ...
        testUserMap.put(testNormalUser.getUserId(), testNormalUser);
        testUserMap.put(testNewUser.getUserId(), testNewUser);
        testUserMap.put(testAdminUser.getUserId(), testAdminUser);
    }
    ...
    void testPublishPost_CreditTooLow() {
        ...
        UserDTO lowCreditUser = new UserDTO();
        ...
        // 2. 模拟依赖行为
        when(userUtil.getUserById(4L)).thenReturn(lowCreditUser);
        ...
        // 4. 验证结果
        assertEquals(PostErrorCode.CREDIT_TOO_LOW, exception.getErrorCode());
    ...
    void testQueryPosts_Success_FromDb() {
        ...
        when(userUtil.getUserMapByIds(anyList())).thenReturn(testUserMap);

...
import xyz.graygoo401.infra.config.MinioConfig;
import xyz.graygoo401.infra.exception.OssException;
...
public class MinioUtil {
    ...
    public String uploadImage(MultipartFile file, OssModuleEnum module, String bucketName) {
        ...
        if (originalFilename == null || !originalFilename.matches(".*\\.(jpg|png|webp)$")) {
            throw new OssException(OssErrorCode.OSS_PICTURE_FORMAT_INVALID, "仅支持 jpg/png/webp 格式图片");
        }
    ...
    public String uploadFile(MultipartFile file, OssModuleEnum module, String bucketName) {
        ...
        if (fileSize > module.getMaxSize()) { // 头像≤1MB
            throw new OssException(OssErrorCode.OSS_FILE_SIZE_INVALID, module.getDesc() + "数据大小不能超过 " +
        ...
        try (InputStream inputStream = file.getInputStream()) {
            ...
            throw new OssException(OssErrorCode.OSS_SERVICE_FAILS);
        } catch (IOException e) {
            log.error("文件流读取失败：{}", e.getMessage());
            throw new BusinessException(SystemErrorCode.FAILURE, "图片处理失败");
        } catch (Exception e) {
            log.error("图片上传失败：{}", e.getMessage());
            throw new BusinessException(SystemErrorCode.FAILURE, "图片上传失败");
        }
    }
    ...
    public void downloadToResponse(String bucketName, String objectName, HttpServletResponse response) {
        try {
            ...
            log.error("文件获取失败：{}", e.getMessage());
            throw new OssException(OssErrorCode.OSS_SERVICE_FAILS, "文件获取失败");
        }
    }
    ...
    public String generatePresignedUrl(String bucketName, String filePath) {
        try {
            ...
            throw new OssException(OssErrorCode.OSS_PRESIGNED_URL_GENERATE_FAILS);
        } catch (Exception e) {
            log.error("MinIO 生成签名 URL 失败：{}", e.getMessage());
            throw new OssException(OssErrorCode.OSS_SERVICE_FAILS, "MinIO 生成签名 URL 失败");
        }
    }
    ...
    public void deleteFile(String bucketName, String objectName) {
        try {
            ...
            throw new OssException(SystemErrorCode.FAILURE, "删除文件失败");
        }
    }
    ...
    public List<DeleteError> deleteFiles(String bucketName, List<String> objectNames) {
        try {
            ...
            throw new OssException(SystemErrorCode.FAILURE, "批量删除文件失败");
        }
    }
    ...
    public List<String> listAllFiles(String bucketName) {
        try {
            ...
            throw new OssException(OssErrorCode.OSS_SERVICE_FAILS, "列出存储桶中的所有文件失败");
        }
    }
    ...
    public StatObjectResponse getObjectStat(String objectName) {
        try {
            ...
            throw new OssException(OssErrorCode.OSS_SERVICE_FAILS, "获取文件元数据失败");
        }
    }
    ...
    public void validateFile(MultipartFile file) {
        // 1. 校验文件是否为空
        if (file.isEmpty()) {
            throw new BusinessException(OssErrorCode.OSS_FILE_NOT_EXISTS,"上传文件不能为空");
        }

        // 2. 获取文件扩展名（小写）
        String originalFilename = file.getOriginalFilename();
        if (originalFilename == null || !originalFilename.contains(".")) {
            throw new BusinessException(OssErrorCode.OSS_FILE_FORMAT_INVALID, "文件名称格式非法，无扩展名");
        }
        String extension = originalFilename.substring(originalFilename.lastIndexOf(".") + 1).toLowerCase();

        // 3. 校验扩展名白名单
        if (!ALLOWED_EXTENSION_TYPES.contains(extension)) {
            throw new BusinessException(OssErrorCode.OSS_FILE_FORMAT_INVALID,
        ...
        if (contentType == null || !ALLOWED_MIME_TYPES.contains(contentType)) {
            throw new BusinessException(OssErrorCode.OSS_FILE_FORMAT_INVALID, "文件 MIME 类型非法：" + contentType + "，允许的类型：" + String.join(",", ALLOWED_MIME_TYPES));
        }

        // 5. （可选）校验文件大小（例如限制 10MB 以内）
        long fileSize = file.getSize();
        if (fileSize > FILE_MAX_SIZE) {
            throw new BusinessException(OssErrorCode.OSS_FILE_SIZE_INVALID,

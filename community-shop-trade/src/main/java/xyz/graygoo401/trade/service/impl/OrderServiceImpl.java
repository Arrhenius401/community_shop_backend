...
import xyz.graygoo401.api.trade.dto.product.ProductStockUpdateDTO;
...
import xyz.graygoo401.common.util.SignUtil;
import xyz.graygoo401.trade.convert.OrderConvert;
import xyz.graygoo401.trade.convert.ProductConvert;
import xyz.graygoo401.trade.dao.entity.Order;
import xyz.graygoo401.trade.dao.entity.Product;
import xyz.graygoo401.trade.dao.mapper.OrderMapper;
    ...
    public OrderDetailDTO createOrder(Long userId, OrderCreateDTO orderCreateDTO) {
        try {
            ...
            if (!ProductStatusEnum.ON_SALE.equals(product.getStatus())) {
                throw new BusinessException(ProductErrorCode.PRODUCT_ALREADY_OFF_SALE, "商品已下架或不可售");
            }
            if (product.getStock() < orderCreateDTO.getQuantity()) {
                throw new BusinessException(ProductErrorCode.PRODUCT_STOCK_INSUFFICIENT,
            ...
            if (diff.compareTo(new BigDecimal("0.01")) >= 0) {
                throw new BusinessException(OrderErrorCode.ORDER_AMOUNT_ABNORMAL, "订单金额异常，请重新下单");
            }
            ...
            if (orderInsertCount != 1) {
                ...
                throw new BusinessException(SystemErrorCode.DATA_INSERT_FAILED, "订单创建失败，请重试");
            }
            ...
            if (stockUpdateCount != 1) {
                throw new BusinessException(SystemErrorCode.DATA_UPDATE_FAILED, "库存更新失败，订单创建回滚");
            }
            ...
            log.error("创建订单异常", e);
            throw new BusinessException(SystemErrorCode.FAILURE, "订单创建异常，请联系客服");
        }
    }
    ...
    public Boolean cancelOrder(Long userId, Long orderId) {
        try {
            ...
            if (userId == null || orderId == null) {
                throw new BusinessException(SystemErrorCode.PARAM_NULL, "用户ID和订单ID不能为空");
            }
            ...
            // 3. 权限校验（买家或管理员可取消）
            UserDTO operator = userUtil.getUserById(userId);
            ...
            if (!isBuyer && !isAdmin) {
                throw new BusinessException(SystemErrorCode.PERMISSION_DENIED, "无权限取消此订单");
            }
            ...
            if (product == null) {
                throw new BusinessException(ProductErrorCode.PRODUCT_NOT_EXISTS, "商品不存在，无法恢复库存");
            }
            ProductStockUpdateDTO stockUpdateDTO = new ProductStockUpdateDTO(product.getProductId(),
                    order.getQuantity(), "订单创建");
            int stockUpdateCount = productService.updateStock(product.getSellerId(), stockUpdateDTO);
            if (stockUpdateCount != 1) {
                throw new BusinessException(SystemErrorCode.DATA_UPDATE_FAILED, "库存恢复失败，取消订单回滚");
            }
...
//            order.setCancelReason(isBuyer ? "买家主动取消" : "管理员操作取消");
            ...
            if (orderUpdateCount != 1) {
                throw new BusinessException(SystemErrorCode.DATA_UPDATE_FAILED, "订单状态更新失败");
            }
            ...
            log.error("取消订单异常", e);
            throw new BusinessException(SystemErrorCode.FAILURE, "取消订单异常，请联系客服");
        }
    }
...
//            order.setPayNo(payCallbackDTO.getPayNo()); // 第三方支付流水号
    ...
    public OrderDetailDTO shipOrder(Long sellerId, Long orderId, OrderShipDTO shipDTO) {
        try {
            ...
            if (sellerId == null || orderId == null || shipDTO == null
                ...
                throw new BusinessException(SystemErrorCode.PARAM_NULL, "参数不完整，发货失败");
            }
            ...
            if (!Objects.equals(order.getSellerId(), sellerId)) {
                throw new BusinessException(SystemErrorCode.PERMISSION_DENIED, "无权限发货，此订单不属于当前卖家");
            }
            ...
            int updateCount = orderMapper.updateById(order);
            if (updateCount != 1) {
                throw new BusinessException(SystemErrorCode.DATA_UPDATE_FAILED, "订单发货状态更新失败");
            }
            ...
            log.error("订单发货异常", e);
            throw new BusinessException(SystemErrorCode.FAILURE, "发货操作异常，请联系客服");
        }
    }
    ...
    public OrderDetailDTO confirmReceive(Long buyerId, Long orderId) {
        try {
            ...
            if (!Objects.equals(order.getBuyerId(), buyerId)) {
                throw new BusinessException(SystemErrorCode.PERMISSION_DENIED, "无权限确认收货，此订单不属于当前买家");
            }
            ...
            int updateCount = orderMapper.updateById(order);
            if (updateCount != 1) {
                throw new BusinessException(SystemErrorCode.DATA_UPDATE_FAILED, "订单确认收货状态更新失败");
            }
            ...
            throw new BusinessException(SystemErrorCode.FAILURE, "确认收货操作异常，请联系客服");
        }
    }
    ...
    public OrderDetailDTO getOrderDetail(Long userId, Long orderId) {
        try {
            ...
            throw new BusinessException(SystemErrorCode.DATA_QUERY_FAILED, "查询订单详情失败，请重试");
        }
    }
    ...
    public PageResult<OrderListItemDTO> getBuyerOrders(Long buyerId, OrderQueryDTO queryDTO) {
        try {
            // 1. 基础参数校验
            if (buyerId == null) {
                throw new BusinessException(SystemErrorCode.PARAM_NULL, "买家ID不能为空");
            }
            if (queryDTO == null) {
...
//                }
            ...
            throw new BusinessException(SystemErrorCode.DATA_QUERY_FAILED, "查询订单列表失败，请重试");
        }
    }
    ...
    public PageResult<OrderListItemDTO> getSellerOrders(Long sellerId, OrderQueryDTO queryDTO) {
        try {
            // 1. 基础参数校验
            if (sellerId == null) {
                throw new BusinessException(SystemErrorCode.PARAM_NULL, "卖家ID不能为空");
            }
            if (queryDTO == null) {
                queryDTO = new OrderQueryDTO();
            }
...
//                }
    ...
    public int autoCloseTimeoutOrders(int timeoutMinutes) {
        try {
            if (timeoutMinutes <= 0) {
                throw new BusinessException(SystemErrorCode.PARAM_ERROR, "超时时间必须大于0");
            }
...
//                    order.setCancelReason("超时未支付，系统自动关闭");
            ...
            throw new BusinessException(SystemErrorCode.FAILURE, "自动关闭订单异常，请联系运维");
        }
    }
    ...
    private OrderDetailDTO OrderToOrderDetailDTO(Order order) {
        ...
        // 3.补充买家信息
        UserDTO buyer = userUtil.getUserById(order.getBuyerId());
        if(buyer != null){
            orderDetail.setBuyer(orderConvert.userDTOToBuyerSimpleDTO(buyer));
        }

        // 4. 补充卖家信息
        UserDTO seller = userUtil.getUserById(order.getSellerId());
        if(seller != null){
            orderDetail.setSeller(orderConvert.userDTOToSellerSimpleDTO(seller));
        }
    ...
    private void validateOrderCreateParam(OrderCreateDTO orderCreateDTO) {
        if (orderCreateDTO == null) {
            throw new BusinessException(SystemErrorCode.PARAM_NULL, "订单创建参数不能为空");
        }
        if (orderCreateDTO.getProductId() == null) {
            throw new BusinessException(SystemErrorCode.PARAM_NULL, "商品ID不能为空");
        }
        if (orderCreateDTO.getQuantity() == null || orderCreateDTO.getQuantity() <= 0) {
            throw new BusinessException(SystemErrorCode.PARAM_ERROR, "购买数量必须大于0");
        }
        if (orderCreateDTO.getTotalAmount() == null || orderCreateDTO.getTotalAmount().compareTo(BigDecimal.ZERO) <= 0) {
            throw new BusinessException(SystemErrorCode.PARAM_ERROR, "订单金额必须大于0");
        }
        if (!StringUtils.hasText(orderCreateDTO.getAddress())) {
            throw new BusinessException(SystemErrorCode.PARAM_NULL, "收货地址不能为空");
        }
        if (orderCreateDTO.getPayType() == null) {
            throw new BusinessException(SystemErrorCode.PARAM_NULL, "支付方式不能为空");
        }
    }
    ...
    private void validateOrderPermission(Long operatorId, Long buyerId, Long sellerId) {
        ...
        if (!isBuyer && !isSeller && !isAdmin) {
            throw new BusinessException(SystemErrorCode.PERMISSION_DENIED, "无权限查看此订单详情");
        }
    }

...
import xyz.graygoo401.api.community.dto.post.*;
...
import xyz.graygoo401.community.dao.mapper.PostMapper;
import xyz.graygoo401.community.exception.error.PostErrorCode;
import xyz.graygoo401.community.service.base.PostService;
import xyz.graygoo401.community.service.base.UserPostLikeService;
...
public class PostServiceImpl extends BaseServiceImpl<PostMapper, Post> implements PostService {
    ...
    public PostDetailDTO publishPost(Long userId, PostPublishDTO postPublishDTO) {
        try {
            ...
            // 2. 校验发布者存在且信用分达标（≥60分）
            UserDTO publisher = userUtil.getUserById(userId);
            if (publisher.getCreditScore() < PUBLISH_MIN_CREDIT) {
                ...
                throw new BusinessException(PostErrorCode.CREDIT_TOO_LOW);
            }
    ...
    public PostDetailDTO updatePost(Long postId, Long userId, PostUpdateDTO postUpdateDTO) {
        try {
            ...
            UserDTO publisher = userUtil.getUserById(updatedPost.getUserId());
    ...
    public Integer updateLikeStatus(PostLikeDTO postLikeDTO) {
        try {
            ...
            if (!PostStatusEnum.NORMAL.equals(post.getStatus())) {
                ...
                throw new BusinessException(PostErrorCode.POST_STATUS_INVALID);
            }
            ...
            if (isLike && !checkDailyLikeLimit(userId)) {
                ...
                throw new BusinessException(PostErrorCode.DAILY_LIKE_TIMES_EXCEED);
            }
    ...
    public Boolean setEssenceOrTop(Long userId, PostEssenceTopDTO postEssenceTopDTO) {
        try {
            ...
            if (!UserRoleEnum.ADMIN.equals(admin.getRole())) {
                log.error("设置帖子精华/置顶失败，无管理员权限，操作人ID：{}，角色：{}",
                        userId, admin.getRole());
                throw new BusinessException(SystemErrorCode.PERMISSION_DENIED);
            }
            ...
            if (isTop && !post.getIsTop()) {
                ...
                if (currentTopCount >= MAX_TOP_POST_COUNT) {
                    ...
                    throw new BusinessException(PostErrorCode.TOP_POST_COUNT_EXCEED);
                }
            }
    ...
    public int batchDeletePosts(Long adminId, List<Long> postIds) {
        try {
            ...
            // 2. 校验管理员权限
            UserDTO admin = userUtil.getUserById(adminId);
    ...
    private void validatePublishParam(PostPublishDTO publishDTO) {
        ...
        if (!StringUtils.hasText(publishDTO.getTitle())) {
            throw new BusinessException(PostErrorCode.POST_TITLE_NULL);
        }
        if (publishDTO.getTitle().length() > MAX_POST_TITLE_LENGTH) {
            throw new BusinessException(PostErrorCode.POST_TITLE_INVALID);
        }
        // 内容非空且长度≤2000字
        ...
        if (publishDTO.getContent().length() > MAX_POST_CONTENT_LENGTH) {
            throw new BusinessException(PostErrorCode.POST_CONTENT_INVALID);
        }
    }
        ...
        if (StringUtils.hasText(imageUrlsJson)) {
            try {
                ...
                if (imageUrlList.size() > MAX_IMAGE_COUNT) {
                    throw new BusinessException(PostErrorCode.POST_IMAGE_TOO_MANY);
                }
            } catch (Exception e) {
                log.error("解析帖子图片URL失败，JSON格式异常：{}", imageUrlsJson, e);
                throw new BusinessException(PostErrorCode.POST_IMAGE_FORMAT_INVALID);
            }
        }
        return imageUrlList;
    }
     ...
     * 构建Post实体
     */
    private Post buildPostEntity(PostPublishDTO publishDTO, Long userId, List<String> imageUrlList, UserDTO publisher) {
        Post post = new Post();
        // 基础信息
...
//        post.setImageUrls(imageUrlList); // 图片URL列表
     ...
     * 豁免管理员
     */
    private boolean isNewUser(UserDTO user) {
        LocalDateTime registerTime = user.getCreateTime();
        LocalDateTime sevenDaysAgo = LocalDateTime.now().minusDays(3);
        return registerTime.isAfter(sevenDaysAgo) && !userUtil.verifyRole(user.getUserId(), UserRoleEnum.ADMIN);
    }

    /**
     * 构建帖子详情DTO（关联发布者脱敏信息）
     */
    private PostDetailDTO buildPostDetailDTO(Post post, UserDTO publisher, boolean isLiked) {
    ...
    private List<PostListItemDTO> convertToPostListItemDTO(List<Post> postList) {
        ...
        Map<Long, UserDTO> publisherMap = userUtil.getUserMapByIds(new ArrayList<>(publisherIds));
        ...
        return postList.stream().map(post -> {
            ...
            // 封装发布者极简信息
            UserDTO publisher = publisherMap.get(post.getUserId());
    ...
    public PostDetailDTO selectPostById(Long userId, Long postId) {
        try {
            ...
            if(post.getStatus() != PostStatusEnum.NORMAL &&
                    (!userUtil.verifyRole(userId, UserRoleEnum.ADMIN) && !userId.equals(post.getUserId()))){
                throw new BusinessException(SystemErrorCode.PERMISSION_DENIED);
            }

            // 4. 封装详情DTO
            UserDTO publisher = userUtil.getUserById(post.getUserId());
            Boolean isliked = userPostLikeService.isLiked(userId, postId);
    ...
    public Boolean updatePostStatus(Long operatorId, Long postId, PostStatusEnum status) {
        ...
        UserDTO user = userUtil.getUserById(operatorId);
